...
const nextConfig = {
    ...
    // `output: 'export'` makes dynamic routes (/writing/dispatches/[slug]) 500
    // with a bogus "missing generateStaticParams()" error. Dev renders the
    // route on-demand instead; `npm run build` still produces the static export.
    ...
    // Emit `out/writing/index.html` style paths so nginx can serve them
