...
// `posts` = dispatches (served under /writing/dispatches); `poems` = poetry.
...
export async function getPostBySlug(
    ...
    // sanitize:false lets raw HTML through (e.g. YouTube <iframe> embeds in a
    // poem). Safe here because all content is first-party — my own .md files in
    // this repo, never untrusted user input.
    const htmlOpts = { sanitize: false };
    const processor = preserveLineBreaks
        ? remark().use(remarkBreaks).use(html, htmlOpts)
        : remark().use(html, htmlOpts);
